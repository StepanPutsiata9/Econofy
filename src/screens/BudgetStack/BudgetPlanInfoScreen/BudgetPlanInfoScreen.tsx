...
function BudgetPlanInfoScreen({ navigation }: BudgetPlanInfoScreenProps) {
  ...
  const recommendationsText = useMemo(() => {
    return planAllInfo?.recommendations
      .map((item, index) => ` ${index + 1}.${item}`)
      .join('\n');
  }, [planAllInfo?.recommendations]);
  return (
    <View
      style={[
        styles.container,
    ...
    >
      ...
      {planInfoLoading && <LoadContainer />}
      {planInfoError && <ErrorMessage />}
      {!planInfoLoading && !planInfoError && (
        <ScrollView>
          <Text style={styles.planTitle}>{planAllInfo!.title}</Text>
          <View style={styles.distributionView}>
            <Text style={styles.distributionText}>Распределение бюджета:</Text>
            <Pie
              shouldAnimated={false}
              data={planAllInfo!.budgetPlan}
          ...
          <View style={styles.analysisView}>
            ...
            <TouchableOpacity onPress={toggleAnalysis}>
              <Animated.View
                style={{ transform: [{ rotate: analysisRotate }] }}
              >
                <Arrow color={'#fff'} />
              </Animated.View>
            </TouchableOpacity>
          ...
          >
            <Text style={styles.contentText}>{planAllInfo?.analysis}</Text>
          </Animated.View>
          ...
          >
            <Text style={styles.contentText}>{recommendationsText}</Text>
          </Animated.View>
